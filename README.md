...
COntext API removes the need for prop drilling, store data on a central context object and retrieve that data in the necessary componnents from the context object, not props.


2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
Actions are objects contain an action type and associated data that are dispatched to reducers after being called.
Reducers then take action and follow the instructions from the action.
Store is an redux object contains the state tree for the entire application.


3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
redux-thunk allows us to use to make API calls from our actions creators.
It changes the action creators by making them perform asynchronous API calls using middleware.

4. What is your favorite state management system you've learned and this sprint? Please explain why!
reducer is my favorite state management system because it is a pure function there is no side effects. 
It separate concerns of state transition based on multiple actions, easy to find and apply. 